...
import AuctionCard from "../../components/auctionCard/auctionCard"; // Assuming this component will handle the "Edit" button
...
const UserAuction = () => {
  ...
  const [itemsPerPage, setItemsPerPage] = useState(15); // As per your latest code
  ...
  // Correctly extract currentUser and token
  const { user: currentUser, token } = useSelector(
    (state) => state.auth.userData || { user: null, token: null } // Ensure your Redux state for auth.userData has user and token
  );
  ...
  const fetchAuctions = useCallback(async () => {
    ...
    if (isMyAuctionsRoute && !token) {
      // If on "my-auctions" route and no token, don't fetch, show login prompt
      setIsLoading(false);
      setAuctionsData([]);
      setTotalAuctionsFromAPI(0);
      setTotalPagesFromAPI(1);
      // Error state could be set here to indicate login is required, or handled by the main return block
      return;
    }
    ...
    try {
      const response = await axios.get(
        ...
        }/auction/get-specific-auctions?${params.toString()}`,
        {
      ...
      // If unauthorized on /my-auctions, backend might send 401/403
      if (
        (err.response?.status === 401 || err.response?.status === 403) &&
        isMyAuctionsRoute
      ) {
        errorMessage =
          "Please log in to view your auctions. Your session might have expired.";
      }
    ...
    currentUser, // Keep currentUser if its absence should prevent any fetch
    ...
    sortBy,
    sortOrder,
    // API_BASE_URL is a constant, typically not needed in useCallback deps unless it can change during component lifecycle
  ]);
  ...
  useEffect(() => {
    ...
    const handleAuctionEvent = (updatedAuctionData) => {
      console.log(
        "Socket: auction event received on MyAuction page",
        updatedAuctionData
      );
    ...
    const handleStatusSpecificEvent = (eventData) => {
      if (eventData.auctionId && eventData.data) {
        // Ensure data object is present
        setAuctionsData((prevAuctions) =>
          prevAuctions.map((auc) =>
            auc._id === eventData.auctionId
              ? { ...auc, ...eventData.data, status: eventData.status } // Make sure eventData.data contains relevant auction fields
              : auc
          )
        );
      } else if (eventData.auctionId && eventData.status) {
        // Fallback if only status is sent
        setAuctionsData((prevAuctions) =>
          prevAuctions.map((auc) =>
            auc._id === eventData.auctionId
              ? { ...auc, status: eventData.status }
              : auc
          )
        );
      }
  ...
  }, [socket, auctionsData, fetchAuctions]);
  ...
  // If on "My Auctions" route, not loading, and no current user (or token was invalid leading to error)
  if (
    isMyAuctionsRouteCheck &&
    !isLoading &&
    (!currentUser || !token || (error && error.includes("log in")))
  ) {
    return (
      <div className="container mx-auto p-8 text-center">
        <h2 className="text-2xl font-semibold text-gray-700">Access Denied</h2>
        <p className="text-gray-600 mt-2">
          {error || "Please log in to view your auctions."}
        </p>
        <Link
          to="/login"
  ...
  if (error && !(isMyAuctionsRouteCheck && (!currentUser || !token))) {
    // Show general error if not the specific login error for my-auctions
    return (
  ...
  return (
    <div className="container mx-auto p-4 md:p-6 lg:p-8">
      <div className="mb-8 flex flex-wrap justify-between items-center text-center md:text-left">
        ...
        {currentUser && ( // Show Create Auction button only if user is logged in
      ...
      {!isLoading && auctionsData.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {auctionsData.map((auction) => {
            // On "/my-auctions" route, backend ensures these are owned.
            // On "/auctions" route, this checks if an auction in the general list is owned by the current user.
            const isOwner = currentUser?.id === auction.createdBy?._id;
            return (
              <AuctionCard
                key={auction._id}
                auction={auction}
                isMyAuctionsPage={isOwner} // True if on my-auctions page (implies owner) OR if on all-auctions page and is owner
              />
            );
          })}
        </div>
